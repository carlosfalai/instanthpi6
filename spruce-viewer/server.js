...
async function syncConversations() {
  ...
  while (hasMore && pageCount < 100) {
    // Limit to 100 pages (20k conversations max)
    ...
    try {
      while (hasMore && attemptCount < 20) {
        const response = await axios.get(
          `${SPRUCE_API_URL}/conversations/${conversationId}/items`,
          {
          ...
          if (item.type === "message") {
            ...
            history.push({
              ...
              content:
                ...
                item.content ||
                item.body ||
                (item.event ? `[${item.event.type}]` : "Media/Event"),
              timestamp:
                item.createdAt || item.timestamp || item.sent_at || new Date().toISOString(),
              senderName:
                item.author?.displayName ||
                item.sender_name ||
                (item.direction === "inbound" || item.is_from_patient ? "Patient" : "Doctor"),
...
app.post("/api/archive/:conversationId", async (req, res) => {
  try {
    ...
    await axios.patch(
      ...
      {
        headers: {
          Authorization: `Bearer ${spruceBearerToken}`,
