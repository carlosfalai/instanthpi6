...
const DEFAULT_TEMPLATES: Omit<MedicalTemplate, 'id' | 'physician_id' | 'created_at' | 'updated_at' | 'usage_count' | 'last_used_at'>[] = [
  ...
  },
  {
    ...
    template_content: "S: Homme demandant l'obtention d'une classe professionnelle 4B. Aucun antécédent médical, aucun usage d'alcool ou de drogues récréatives. Porte des lunettes. Pas de symptômes cardiaques. Pas de médecin de famille attitré. Aucun antécédent de conduite avec facultés affaiblies. Questionnaire CAGE et critères de dépendance tous négatifs.\n\nO: Auto-questionnaire complété, aucune alerte clinique.\n\nA: Aucun facteur médical ou psychiatrique limitant pour l'obtention de la classe 4B.\n\nP: Déclaration de santé favorable. Attestation à émettre.",
    is_enabled: true,
    is_default: false,
  },
  {
  ...
  // Work Leave (1)
  {
    ...
    case_type: "gastroenteritis",
    template_content: "Arrêt de travail médical du 14/10/2025 au 16/10/2025 inclus pour gastro-entérite aiguë nécessitant repos et hydratation à domicile. Reprise du travail prévue le 17/10/2025 sous réserve d'amélioration clinique.",
    is_enabled: true,
    is_default: true,
  },
  // Patient Messages (5)
...
export default function MedicalTemplatesManager() {
  ...
  const setDefaultTemplate = async (templateId: string, category: string, caseType: string | null) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
