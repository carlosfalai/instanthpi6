...
export function EnhancedPatientIntakeForm() {
  ...
  const steps = [
    ...
    },
    {
      ...
      icon: Heart,
      description: "Chronic conditions and allergies",
    },
    {
      id: 3,
      title: "Current Symptoms",
      icon: Stethoscope,
      description: "Describe your current condition",
    },
    {
      id: 4,
    ...
    },
    {
      ...
      icon: CheckCircle,
      description: "Emergency contacts and notes",
    },
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save to database
      const { error } = await supabase.from("consultations").insert([patientData]);
              ...
              return (
                <div key={step.id} className="flex items-center">
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`w-16 h-0.5 mx-2 ${isCompleted ? "bg-green-600" : "bg-gray-300"}`}
                    />
                  )}
                </div>
              );
