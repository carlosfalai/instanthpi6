...
import { Bot, Send, X, Sparkles, Languages, Eraser, Zap, FileText, Pill, ArrowRight } from 'lucide-react';
...
export function EliteAIChat({ isOpen, onClose, currentSections, onUpdateSections }: EliteAIChatProps) {
    ...
    const [activeDoc, setActiveDoc] = useState<{ title: string, content: string }>({ title: '', content: '' });
    ...
    useEffect(() => {
        ...
        const planSection = currentSections.find(s => s.id === 'plan' || s.id === 'assessment' || s.id === 'soap');
        const content = planSection?.content.toLowerCase() || "";

        // Predict: Referral
        if (content.includes('refer') || content.includes('specialist') || content.includes('ent') || content.includes('cardio')) {
            newSuggestions.push({
                id: 'referral',
        ...
        if (content.includes('mg') || content.includes('po') || content.includes('bid') || content.includes('tid') || content.includes('medication') || content.includes('phosphomycin')) {
            newSuggestions.push({
                id: 'rx',
        ...
        // Predict: Work Note
        if (content.includes('work') || content.includes('school') || content.includes('off') || content.includes('leave')) {
            newSuggestions.push({
                id: 'note',
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (lowerInput.includes("french") || lowerInput.includes("translate")) {
                    responseContent = "Je suis prêt. I have translated the Patient Message and Plan to French as requested.";
                    ...
                    responseContent = "I can help you refine the report. Try asking me to 'Translate to French' or 'Make it concise'.";
                }
            ...
            {suggestions.length > 0 && (
                <div className="p-3 bg-primary/10 border-b border-primary/20">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] font-bold text-primary uppercase tracking-wider">Suggested Next Steps</span>
                    </div>
                    <div className="space-y-2">
                        {suggestions.map(action => (
                            ...
                            >
                                ...
                                <div className="text-[10px] text-muted-foreground ml-6 mt-0.5">{action.description}</div>
                            </button>
                        ))}
                    </div>
                </div>
