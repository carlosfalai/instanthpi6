...
export function EliteSpruceMessenger({ isOpen, onClose, initialDraft = "", recipientName }: EliteSpruceMessengerProps) {
    ...
    const loadConversations = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Fallback mock data
                setConversations([
                    { id: '1', patient_name: 'Jean Tremblay', last_message: 'Merci docteur', updated_at: new Date().toISOString() },
                    { id: '2', patient_name: 'Marie Curie', last_message: 'Quand est le RDV?', updated_at: new Date().toISOString() },
                ]);
            }
        } catch (error) {
    ...
    const handleUndo = (id: string) => {
        ...
        if (msgToRestore) {
            ...
            setActiveConversation(conversations.find(c => c.id === msgToRestore.conversationId) || null);
        }
                        ...
                        {conversations.map(conv => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeConversation?.id === conv.id ? "bg-primary/20 border border-primary/20" : "hover:bg-white/5 border border-transparent"
                                )}
                            >
                ...
                <div className="flex-1 flex flex-col bg-[#050505] relative">
                        ...
                        {queuedMessages.length > 0 && (
                            <div className="mb-6 space-y-2">
                                ...
                                {queuedMessages.map(msg => {
                                    ...
                                    return (
                                        <div key={msg.id} className="bg-primary/10 border border-primary/20 rounded-xl p-3 flex items-center gap-4 animate-in slide-in-from-top-2">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="text-xs font-bold text-primary mb-0.5">Auto-sending to {msg.patientName}...</div>
                    ...
                    <div className="p-4 border-t border-white/10 bg-black/20">
                        <div className="mb-2 flex justify-between items-center">
                            <span className="text-xs font-mono text-primary uppercase tracking-wider">AI Draft Content</span>
                        </div>
                        <textarea
