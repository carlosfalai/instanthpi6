...
const STANDARD_BLOCKS = {
  HPI_Confirmation_Summary: {
    ...
    example:
      "Juste pour confirmer avec vous avant de continuer; vous êtes un homme de 45 ans présentant depuis ce matin une douleur thoracique aiguë, localisée côté gauche, aggravée par la respiration profonde, soulagée par le repos; Est-ce que ce résumé est exact ?",
    enabled: true,
  },
  ...
  Medications_ReadyToUse_Prescriptions: {
    ...
    example:
      "Paracétamol 1000mg, comprimé, voie orale, 3 fois par jour, 7 jours, 21 comprimés, 0 renouvellement, à prendre avec un verre d'eau.\nIbuprofène 400mg, comprimé, voie orale, 2 fois par jour, 5 jours, 10 comprimés, 0 renouvellement, à prendre avec les repas.",
    enabled: true,
  },
  Lab_Works: {
    name: "Lab Works",
  ...
  Workplace_Modifications: {
    ...
    example:
      "Éviter les efforts physiques importants, travail de bureau privilégié, pauses fréquentes, éviter le port de charges lourdes.",
    enabled: true,
  },
  ...
  Patient_Message: {
    ...
    example:
      "Bonjour, nous avons bien reçu vos informations. Votre douleur thoracique nécessite une évaluation médicale en personne. Veuillez vous présenter aux urgences ou prendre rendez-vous avec votre médecin de famille dans les plus brefs délais.",
    enabled: true,
  },
};
                ...
                <DialogContent className="max-w-2xl">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          setNewBlock({ ...newBlock, enabled: checked })
                        }
                      />
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <Button onClick={createCustomBlock}>Créer le Bloc</Button>
                    </div>
                  </div>
                </DialogContent>
              </Dialog>
              <Button onClick={generateAllBlocks} disabled={isGenerating || !patientId} size="sm">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => deleteCustomBlock(block.id)}>
          ...
          <CardContent className="space-y-4">
            {Object.entries(generatedBlocks).map(([blockId, content]) => (
              <div key={blockId} className="border rounded-lg p-4">
                ...
                <pre className="text-sm bg-gray-50 p-3 rounded whitespace-pre-wrap">{content}</pre>
              </div>
            ))}
          </CardContent>
        </Card>
