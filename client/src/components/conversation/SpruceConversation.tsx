...
export default function SpruceConversation({
  ...
  const { 
    ...
    queryFn: async () => {
      try {
        const response = await axios.get(`/api/patients/${patientId}/messages`);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Send message to API
      await axios.post(`/api/spruce/messages`, {
        patientId,
        message: messageText,
        messageType: 'GENERAL'
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {messagesLoading ? "Loading..." : `Last active: ${messages.length > 0 ? formatTimestamp(messages[messages.length - 1].timestamp) : "Unknown"}`}
            </p>
          </div>
        </div>
            ...
            <div key={dateKey} className="space-y-4">
              ...
              {messagesByDate[dateKey].map(message => (
                ...
                >
                  ...
                  >
                    {!message.isFromPatient && (
                      <div className="text-xs text-gray-400 mb-1">{message.sender || doctorName}</div>
                    )}
