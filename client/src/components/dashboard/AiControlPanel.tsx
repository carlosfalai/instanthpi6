...
export default function AiControlPanel() {
  ...
  const hasChanges =
    settings && Object.keys(localSettings).some((key) => localSettings[key] !== settings[key]);
      ...
      <CardHeader className="bg-slate-50">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Control which AI features are active during patient consultations
            </CardDescription>
          </div>
      ...
      <CardContent className="pt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {aiFeatures.map((feature) => (
            <div
              key={feature.id}
              className="flex items-start space-x-4 p-4 rounded-md border hover:bg-gray-50 transition-colors"
            >
