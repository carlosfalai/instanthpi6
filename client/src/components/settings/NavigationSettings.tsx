...
const SortableNavItem = ({
  ...
  item: NavItem;
  onToggleChange: (id: string, visible: boolean) => void;
}) => {
...
const NavigationSettings: React.FC<NavigationSettingsProps> = ({ currentUser }) => {
        ...
        body: JSON.stringify({
          navPreferences: {
            ...
            showChronicConditions:
              preferences.navItems.find((item) => item.id === "chronicConditions")?.visible ?? true,
            showMedicationRefills:
              preferences.navItems.find((item) => item.id === "medicationRefills")?.visible ?? true,
            showUrgentCare:
              preferences.navItems.find((item) => item.id === "urgentCare")?.visible ?? true,
          },
        }),
      });
      ...
      const updatedItems = navItems.map((item) => {
        if (item.id === active.id) {
          ...
          ((activeItem.order < overItem.order &&
            item.order > activeItem.order &&
            item.order <= overItem.order) ||
            (activeItem.order > overItem.order &&
              item.order < activeItem.order &&
              item.order >= overItem.order))
        ) {
  ...
  const handleToggleChange = (id: string, visible: boolean) => {
    const updatedItems = navItems.map((item) => (item.id === id ? { ...item, visible } : item));
    setNavItems(updatedItems);
  };
  ...
  const handleResetToDefaults = () => {
    ...
    toast({
      ...
      description:
        "Navigation settings have been reset to default values. Click Save to apply changes.",
    });
  };
      ...
      <CardContent>
        <div className="space-y-6">
          <div>
            ...
            >
              <SortableContext
                items={primaryItems.map((item) => item.id)}
          ...
          <div>
            ...
            >
              <SortableContext
                items={secondaryItems.map((item) => item.id)}
                strategy={verticalListSortingStrategy}
              >
                <div className="space-y-2 mb-4">
