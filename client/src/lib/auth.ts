...
function diagnoseOAuthConfig() {
  ...
  if (supabaseUrl && !supabaseUrl.startsWith('https://')) issues.push('VITE_SUPABASE_URL is not a valid HTTPS URL');
  if (!supabaseAnonKey) issues.push('VITE_SUPABASE_ANON_KEY environment variable is missing');
  if (supabaseAnonKey && supabaseAnonKey.length < 50) issues.push('VITE_SUPABASE_ANON_KEY appears too short - may be invalid');
