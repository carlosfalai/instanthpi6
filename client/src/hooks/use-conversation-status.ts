...
export function useConversationStatus(messages: Message[], patient?: Patient | null) {
  ...
  useEffect(() => {
    ...
    const askingForPharmacy = sortedMessages.some(m => 
      !m.isFromPatient && 
    ...
    const proposingTreatment = sortedMessages.some(m => 
      !m.isFromPatient && 
    ...
    const patientAcknowledged = proposingTreatment && lastPatientMessage && 
    ...
    const askingClarification = lastDoctorMessage &&
    ...
    if (sortedMessages.length <= 2) {
      ...
      const planContent = lastDoctorMessage?.content.split('\n')
        .find(line => 
