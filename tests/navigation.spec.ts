...
test.describe("Navigation & Routes E2E Tests", () => {
    ...
    for (const route of routes) {
      ...
      if (is404) {
        ...
        await page.screenshot({
          path: `screenshots/route-404-${route.path.replace("/", "")}.png`,
          fullPage: true,
        });
      } else {
        ...
        await page.screenshot({
          path: `screenshots/route-${route.path.replace("/", "")}.png`,
          fullPage: true,
        });
      }
    }
  ...
  test("Navigation buttons from dashboard work", async ({ page }) => {
    ...
    for (const button of navigationButtons) {
      ...
      const navButton = page
        .locator(`button:has-text("${button.text}"), a:has-text("${button.text}")`)
        .first();
  ...
  test("Landing page navigation to doctor/patient portals", async ({ page }) => {
    ...
    const doctorPortalLink = page
      .locator(
        'a:has-text("Doctor"), button:has-text("Doctor"), a:has-text("Physician"), text=/Doctor Portal|Physician Portal/i'
      )
      .first();
    ...
    if ((await doctorPortalLink.count()) > 0) {
      ...
      await page.screenshot({
        path: "screenshots/navigation-to-doctor-portal.png",
        fullPage: true,
      });
    }
    ...
    const patientPortalLink = page
      .locator(
        'a:has-text("Patient"), button:has-text("Patient"), text=/Patient Portal|Patient Intake/i'
      )
      .first();
    ...
    if ((await patientPortalLink.count()) > 0) {
      ...
      await page.screenshot({
        path: "screenshots/navigation-to-patient-portal.png",
        fullPage: true,
      });
    }
  });
  ...
  test("Protected routes redirect to login", async ({ page }) => {
    ...
    const protectedRoutes = ["/doctor-dashboard", "/patients", "/doctor-profile"];
