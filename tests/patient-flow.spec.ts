...
test.describe("Patient Flow E2E Tests", () => {
  ...
  test("Complete patient intake flow - form to print document", async ({ page }) => {
    ...
    const genderField = page
      .locator('select:has-text("Gender"), input[placeholder*="Gender" i], select:has-text("Sexe")')
      .first();
    ...
    const reasonField = page
      .locator(
        'input[placeholder*="reason" i], textarea[placeholder*="reason" i], input[placeholder*="motif" i], textarea[placeholder*="motif" i]'
      )
      .first();
    ...
    const symptomField = page
      .locator('textarea[placeholder*="symptom" i], textarea[placeholder*="describe" i]')
      .first();
    ...
    const questions = page.locator(
      'textarea[placeholder*="answer" i], input[placeholder*="answer" i], textarea[placeholder*="réponse" i]'
    );
    ...
    if (questionCount > 0) {
      ...
      const saveButton = page
        .locator(
          'button:has-text("Save"), button:has-text("Submit"), button:has-text("Sauvegarder")'
        )
        .last();
      ...
      const printButton = page.locator(
        'button:has-text("Print"), button:has-text("Open"), button:has-text("Document")'
      );
      ...
      if ((await printButton.count()) > 0) {
        ...
        await page.screenshot({
          path: "screenshots/patient-flow-no-print-button.png",
          fullPage: true,
        });
      }
    } else {
  ...
  test("Patient form validation", async ({ page }) => {
    ...
    const submitButton = page
      .locator('button:has-text("Submit"), button:has-text("Soumettre")')
      .first();
    if ((await submitButton.count()) > 0) {
      ...
      const errorMessages = page.locator(
        "text=/required|obligatoire|please fill|veuillez remplir/i"
      );
