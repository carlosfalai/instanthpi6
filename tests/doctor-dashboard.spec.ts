...
test.describe("Doctor Dashboard E2E Tests", () => {
  ...
  test("Doctor dashboard loads with all sections", async ({ page }) => {
    ...
    const expectedSections = ["Search", "Patient", "Spruce", "File", "Recent", "Medical"];
  ...
  test("Patient search functionality", async ({ page }) => {
    ...
    const searchInput = page
      .locator(
        'input[placeholder*="patient" i], input[placeholder*="search" i], input[placeholder*="ID" i]'
      )
      .first();
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const searchButton = page
        .locator('button:has-text("Search"), button:has-text("Chercher")')
        .first();
  ...
  test("Spruce integration section", async ({ page }) => {
    ...
    if ((await spruceSection.count()) > 0) {
      ...
      const spruceContent = page
        .locator("text=/conversation|message|patient/i")
        .and(page.locator('[class*="spruce" i], [id*="spruce" i]').locator(".."));
  ...
  test("File management functionality", async ({ page }) => {
    ...
    if ((await fileSection.count()) > 0) {
      ...
      const deleteButton = page
        .locator('button:has-text("Delete"), button:has-text("Clean")')
        .first();
  ...
  test("Recent consultations list", async ({ page }) => {
    ...
    if ((await recentSection.count()) > 0) {
      ...
      await page.screenshot({
        path: "screenshots/dashboard-recent-consultations.png",
        fullPage: true,
      });
    } else {
