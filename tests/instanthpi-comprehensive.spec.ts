...
test.describe('InstantHPI Comprehensive Testing', () => {
  ...
  test('3. Patient Intake Form Testing', async ({ page }) => {
    ...
    if (intakePageFound) {
      ...
      const medicalFields = formElements.filter(field => {
        ...
        return name.includes('symptom') || name.includes('medical') ||
               name.includes('history') || name.includes('condition') ||
               placeholder.includes('symptom') || placeholder.includes('medical');
      });
    ...
    for (const path of dashboardPaths) {
      try {
        ...
        if (currentUrl.includes('login') || currentUrl.includes('auth')) {
          ...
          const dashboardElements = await page.$$eval('[class*="dashboard"], [id*="dashboard"], [data-testid*="dashboard"]', elements =>
            elements.map(el => ({
              tag: el.tagName,
              class: el.className,
              id: el.id,
  ...
  test('5. Chrome Extension Integration Check', async ({ page }) => {
    ...
    for (const keyword of extensionKeywords) {
      const elements = await page.$$eval('*', (elements, kw) =>
        elements.filter(el => {
          const text = el.textContent?.toLowerCase() || '';
    ...
    const instructionElements = await page.$$eval('*', elements =>
      elements.filter(el => {
        const text = el.textContent?.toLowerCase() || '';
        return text.includes('install') && (text.includes('extension') || text.includes('chrome'));
      }).map(el => ({
        tag: el.tagName,
  ...
  test('6. Netlify Functions and Integrations Discovery', async ({ page }) => {
    ...
    page.on('request', request => {
      ...
      if (url.includes('/api/') || url.includes('/.netlify/') ||
          url.includes('/functions/') || url.includes('lambda')) {
        apiRequests.push({
          url: url,
    ...
    const integrationHints = await page.evaluate(() => {
      ...
      scripts.forEach(script => {
        ...
        if (src.includes('google') || src.includes('stripe') ||
            src.includes('twilio') || src.includes('sendgrid') ||
            src.includes('supabase') || src.includes('firebase')) {
  ...
  test('7. Complete Workflow Test', async ({ page }) => {
    ...
    const patientLinks = await page.$$eval('a', elements =>
      elements.filter(el => {
        const text = el.textContent?.toLowerCase() || '';
        const href = el.getAttribute('href') || '';
        return text.includes('patient') || text.includes('start') ||
               text.includes('begin') || href.includes('patient');
      }).map(el => ({ text: el.textContent, href: el.getAttribute('href') }))
    );
    ...
    if (patientLinks.length > 0) {
      try {
        ...
        if (formFields.length > 0) {
          for (const field of formFields.slice(0, 5)) { // Limit to first 5 fields
            try {
  ...
  test('8. Performance and Accessibility Check', async ({ page }) => {
    ...
    const accessibilityFeatures = await page.evaluate(() => {
      const features = {
        ...
        focusableElements: Array.from(document.querySelectorAll('a, button, input, textarea, select')).length,
    ...
    await takeScreenshot(page, 'accessibility-performance', 'Accessibility and performance check completed');
  });
});
