...
test.describe('Inbox Page - Spruce Conversations', () => {
  ...
  test('Inbox page loads and fetches Spruce conversations', async ({ page }) => {
    ...
    const hasError = await page.locator('text=/Failed to load|credentials not configured|error/i').count() > 0;
  ...
  test('Conversation history endpoint responds correctly', async ({ page }) => {
    ...
    const conversationsResponse = await page.request.get(`${BASE_URL}/api/spruce-conversations-all`);
    ...
    if (conversationsResponse.status() === 200) {
      ...
      console.log('[Test] Cannot test history endpoint - conversations API returned:', conversationsResponse.status());
    }
  });
});
