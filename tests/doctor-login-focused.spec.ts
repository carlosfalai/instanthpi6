  ...
  test('Doctor Login and Google Sign-in Analysis', async ({ page }) => {
    ...
    try {
      ...
      const formElements = await page.evaluate(() => {
        const elements = Array.from(document.querySelectorAll('input, button, form, [role="button"]'));
      ...
      formElements.forEach((el, index) => {
        console.log(`${index + 1}. ${el.tag} - ${el.text || 'No text'} (type: ${el.type}, class: ${el.className})`);
      });
      ...
      const authLinks = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('a[href*="oauth"], a[href*="auth"], a[href*="login"], a[href*="google"]'));
      ...
      if (googleElements.length > 0) {
        const clickableGoogle = googleElements.find(el =>
          el.tag === 'BUTTON' || el.tag === 'A' || el.className.includes('button')
        );
