...
export class PriorityAIService {
  ...
  async trainPriorityModel(userId: number): Promise<void> {
    ...
    if (userInteractions.length < 20) {
      console.log(`Not enough interactions (${userInteractions.length}) to train model for user ${userId}`);
      return;
    }
    ...
    const timePatternWeights: Record<string, { morning: number, afternoon: number, evening: number }> = {};
    ...
    // Normalize weights
    const totalTaskTypeInteractions = Object.values(taskTypeWeights).reduce((sum, count) => sum + count, 0);
  ...
  async getPrioritizedTasksForUser(userId: number): Promise<any[]> {
    ...
    const models = await db
      ...
      .from(priorityModels)
      .where(and(
        eq(priorityModels.userId, userId),
        eq(priorityModels.active, true)
      ));
  ...
  async getAllTasksWithDefaultPriority(userId: number): Promise<any[]> {
    ...
    const prioritizedTasks = tasks.map(task => {
      ...
      const ageInDays = Math.max(0, (Date.now() - new Date(task.createdAt).getTime()) / (1000 * 60 * 60 * 24));
      if (ageInDays < 1) {
        priorityScore += 10;
      } else if (ageInDays < 3) {
        priorityScore += 5;
