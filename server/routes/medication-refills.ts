...
router.get('/', async (req: Request, res: Response) => {
  try {
    const refillRequests = await db.select().from(medicationRefills).orderBy(medicationRefills.dateReceived);
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a medical assistant AI that analyzes PDF documents to identify medication refill requests. Extract key information and return it in JSON format."
        },
        {
          role: "user",
