...
router.post('/generate-text', async (req, res) => {
  try {
    ...
    const result = await anthropicUtils.generateText(
      prompt,
      model,
      maxTokens
    );
...
router.post('/summarize-text', async (req, res) => {
  try {
    ...
    const result = await anthropicUtils.summarizeText(
      text,
      wordLimit,
      model
    );
...
router.post('/analyze-sentiment', async (req, res) => {
  try {
    ...
    const result = await anthropicUtils.analyzeSentiment(
      text,
      model
    );
...
router.post('/analyze-image', async (req, res) => {
  try {
    ...
    const result = await anthropicUtils.analyzeImage(
      imageBase64,
      prompt,
      model
    );
...
router.post('/generate-medical-documentation', async (req, res) => {
  try {
    ...
    const result = await anthropicUtils.generateMedicalDocumentation(
      patientData,
      options,
      model
    );
...
router.post('/generate-treatment-plan', async (req, res) => {
  try {
    ...
    const result = await anthropicUtils.generateTreatmentPlan(
      diagnosis,
      patientDetails,
      model
    );
