...
pseudonymRouter.post("/", async (req, res) => {
  try {
    ...
    // Create new link
    const [newLink] = await db.insert(pseudonymLinks).values(linkData).returning();
...
pseudonymRouter.delete("/:id", async (req, res) => {
  try {
    ...
    // Delete the link
    await db.delete(pseudonymLinks).where(eq(pseudonymLinks.id, id));
  ...
  try {
    ...
    for (const pseudonym of potentialPseudonyms) {
      ...
      const existingLink = await db
        ...
        .from(pseudonymLinks)
        .where(
          and(eq(pseudonymLinks.pseudonym, pseudonym), eq(pseudonymLinks.patientId, patientId))
        )
        .limit(1);
