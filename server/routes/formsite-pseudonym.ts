...
import { mapFormDataToTemplateVariables, fillTemplate, HPI_CONFIRMATION_TEMPLATE } from '../utils/medicalTemplates';
...
router.get('/lookup/:pseudonym', async (req, res) => {
  try {
      ...
      for (const key in items) {
        if (typeof key === 'string') {
          ...
          if (fieldId === PSEUDONYM_FIELD_ID) {
            ...
            const value = typeof itemValue === 'object' && itemValue?.value ? itemValue.value : itemValue;
    ...
    try {
      ...
      const detailResponse = await formsiteApi.get(`/forms/${FORMSITE_FORM_ID}/results/${matchingSubmission.id}`);
      ...
      try {
        ...
        console.error('Error using template system, falling back to OpenAI generation:', templateError);
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            { role: "system", content: "You are a medical transcription AI that produces structured HTML documents from patient data." },
