...
spruceWebhooksRouter.post('/message-created', verifyWebhookSignature, async (req: Request, res: Response) => {
  try {
...
spruceWebhooksRouter.post('/conversation-updated', verifyWebhookSignature, async (req: Request, res: Response) => {
  try {
    const { data, type } = req.body;
...
spruceWebhooksRouter.post('/contact-updated', verifyWebhookSignature, async (req: Request, res: Response) => {
  try {
    const { data, type } = req.body;
...
spruceWebhooksRouter.post('/generic', verifyWebhookSignature, async (req: Request, res: Response) => {
  try {
    const { data, type } = req.body;
  ...
  try {
    ...
    const webhooksToRegister = [
      ...
      },
      {
        ...
        events: ['appointment.created', 'appointment.updated', 'appointment.cancelled', 'call.started', 'call.ended']
      }
    ];
