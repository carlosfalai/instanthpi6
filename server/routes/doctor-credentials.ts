...
router.get("/credentials", async (req: Request, res: Response) => {
  try {
    ...
    const masked = {
      ...
      spruce_access_id_masked: data.spruce_access_id
        ? maskCredential(decryptCredential(data.spruce_access_id))
        : null,
...
router.delete("/credentials", async (req: Request, res: Response) => {
  try {
    ...
    const { error } = await supabase.from("doctor_credentials").delete().eq("user_id", userId);
